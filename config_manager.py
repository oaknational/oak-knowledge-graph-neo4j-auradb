...
class ConfigManager:
    ...
    def _validate_config(self, config: Dict[str, Any]) -> None:
        ...
        if not isinstance(config['materialized_views'], dict):
            raise ConfigurationError("materialized_views must be a dict mapping view names to field lists")

        # Validate join_strategy (now required)
        self._validate_join_strategy(config['join_strategy'], config['materialized_views'])
        ...
        if 'schema_mapping' not in config:
            if 'node_mappings' in config or 'relationship_mappings' in config:
                ...
                raise ConfigurationError("Configuration must contain 'schema_mapping' or legacy 'node_mappings'/'relationship_mappings'")
                ...
                if label:
                    ...
                    for prop_name, prop_config in properties.items():
                        if isinstance(prop_config, dict):
                            ...
                            if source_field:
                                schema_mapping['nodes'][label]['properties'][prop_name] = source_field
                        else:
                            schema_mapping['nodes'][label]['properties'][prop_name] = prop_config
                ...
                if rel_type:
                    ...
                    for prop_name, prop_config in properties.items():
                        if isinstance(prop_config, dict):
                            source_field = prop_config.get('source_field')
                            if source_field:
                                schema_mapping['relationships'][rel_type]['properties'][prop_name] = source_field
                        else:
                            schema_mapping['relationships'][rel_type]['properties'][prop_name] = prop_config
    ...
    def _validate_join_strategy(self, join_strategy: Dict[str, Any], materialized_views: Dict[str, Any]) -> None:
        ...
        if strategy_type not in ['single_source', 'multi_source_join']:
            raise ConfigurationError("join_strategy.type must be 'single_source' or 'multi_source_join'")
        ...
        if primary_mv and primary_mv not in materialized_views:
            raise ConfigurationError(f"join_strategy.primary_mv '{primary_mv}' not found in materialized_views")

        # Validate joins if present
        joins = join_strategy.get('joins', [])
        if strategy_type == 'multi_source_join' and not joins:
            raise ConfigurationError("multi_source_join strategy requires at least one join configuration")

        for i, join_config in enumerate(joins):
            # Validate mv exists
            join_mv = join_config.get('mv')
            if join_mv not in materialized_views:
                raise ConfigurationError(f"Join {i}: mv '{join_mv}' not found in materialized_views")

            # Validate join_type
            join_type = join_config.get('join_type', 'inner')
            if join_type not in ['inner', 'left', 'right', 'outer']:
                raise ConfigurationError(f"Join {i}: join_type must be 'inner', 'left', 'right', or 'outer'")

            # Validate on clause
            on_clause = join_config.get('on')
            if not on_clause or not isinstance(on_clause, dict):
                raise ConfigurationError(f"Join {i}: 'on' clause is required and must be a dict")

            if 'left_key' not in on_clause or 'right_key' not in on_clause:
                raise ConfigurationError(f"Join {i}: 'on' clause must contain 'left_key' and 'right_key'")